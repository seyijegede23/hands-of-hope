...
const Admin = () => {
  const [view, setView] = useState<"login" | "forgot" | "reset" | "dashboard">(
    "login"
  );
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        "https://heartofhopeserver1.onrender.com/auth/login",
        {
          method: "POST",
  ...
  const fetchData = async () => {
    const res = await fetch(
      "https://heartofhopeserver1.onrender.com/admin/data",
      {
        method: "POST",
      }
    );
  ...
  const requestApproval = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        "https://heartofhopeserver1.onrender.com/admin/request-broadcast-otp",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const sendNewsletter = async () => {
    ...
    try {
      const res = await fetch(
        "https://heartofhopeserver1.onrender.com/send-newsletter",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleAddAdmin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        "https://heartofhopeserver1.onrender.com/admin/add-user",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleDeleteAdmin = async (id: string) => {
    ...
    try {
      const res = await fetch(
        "https://heartofhopeserver1.onrender.com/admin/delete-user",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const requestReset = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        "https://heartofhopeserver1.onrender.com/auth/forgot-password",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const confirmReset = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        "https://heartofhopeserver1.onrender.com/auth/reset-password",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          ...
          {view === "forgot" && (
            <form onSubmit={requestReset} className="space-y-4 text-center">
              ...
              <button
                ...
                className="flex items-center gap-2 justify-center text-sm text-slate-500 w-full"
              >
      ...
      <aside className="w-64 bg-slate-900 text-white p-6 hidden md:block shrink-0">
        <div className="mb-8">
          ...
          {currentRole === "superadmin" && (
            <span className="bg-primary text-xs px-2 py-0.5 rounded text-white mt-1 inline-block">
              Super Admin
            </span>
          )}
        </div>
        ...
        <nav className="space-y-2">
          {["overview", "volunteers", "messages", "newsletter", "team"].map(
            (tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={cn(
                  "w-full text-left p-3 rounded capitalize flex items-center gap-3",
                  activeTab === tab
          ...
          <button
            ...
            className="w-full text-left p-3 text-red-400 hover:bg-slate-800 rounded mt-8 flex items-center gap-3"
          >
                  ...
                  <div className="space-y-3">
                    {!showOtpInput ? (
                      <Button
                        ...
                        className="w-full"
                      >
                        {isSending ? "Requesting..." : "Request Approval"}
                      </Button>
                    ) : (
            ...
            <div className="bg-white p-6 rounded-2xl border shadow-sm">
              ...
              <div className="max-h-[500px] overflow-y-auto">
                {subscribers.map((sub: any) => (
                  <div
                    key={sub._id}
                    className="p-3 border-b flex justify-between group"
                  >
                    <span>{sub.email}</span>
                    <button
                      onClick={() => {
          ...
          <div className="max-w-4xl">
            {currentRole === "superadmin" ? (
              ...
              <div className="bg-blue-50 text-blue-800 p-4 rounded-xl mb-8 flex items-center gap-3">
                <ShieldAlert className="w-5 h-5" /> Super Admin access required to
                add/remove users.
              </div>
            )}
                    ...
                    <div>
                      <p className="font-bold">
                        ...
                        {admin.role === "superadmin" && (
                          <Shield className="w-3 h-3 inline text-primary" />
                        )}
                      </p>
